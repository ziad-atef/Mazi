...
#include "EXTINT.h"
/**
 * @brief  Logic of the function
 *  - Disable the interrupt
 *  - Set GPIO_RCC register depending on PIN number
 *  - Set Interrupt Pin to be input pin 
 *  - Set the triggering edge of the interrupt
...
void _EXTINT_init(unsigned short PORT,unsigned short PIN, triggeringEdge interrupt_edge, state default_state, void (*intHandler)(void))
{
	...
	//Enable GPIO_RCC
	if(PORT == PA) RCC->APB2ENR |= RCC_APB2ENR_IOPAEN;
	if(PORT == PB) RCC->APB2ENR |= RCC_APB2ENR_IOPBEN;
	if(PORT == PC) RCC->APB2ENR |= RCC_APB2ENR_IOPCEN;
	...
	else if(default_state == PULL_DOWN){ //needs to change
		init_GP(PORT,PIN,IN,I_PP);
	}
	// Enable the global interrupt function
	switch(PIN){ //choose IRQ  based on the EXTI_LINE that this PIN belongs to
		case 0:
